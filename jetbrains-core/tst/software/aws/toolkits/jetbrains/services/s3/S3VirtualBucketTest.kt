...
class S3VirtualBucketTest {
    ...
    fun deleteObjects() {
        ...
        val sut = S3VirtualBucket(Bucket.builder().name("TestBucket").build(), null, s3Client, projectRule.project)
        runBlocking {
    ...
    fun downloadObject() {
        ...
        val sut = S3VirtualBucket(testBucket, null, s3Client, projectRule.project)
        runBlocking {
    ...
    fun getUrl() {
        ...
        AwsClientManager().getClient<S3Client>(awsConnectionManager.activeCredentialProvider, awsConnectionManager.activeRegion).use {
            val sut = S3VirtualBucket(Bucket.builder().name("test-bucket").build(), null, it, projectRule.project)
    ...
    fun handleDeletedBucket() {
        ...
        val testBucket = Bucket.builder().name("TestBucket").build()
        val s3VirtualBucket = S3VirtualBucket(testBucket, null, s3Mock, projectRule.project)
        runInEdtAndWait {
            runWriteAction {
