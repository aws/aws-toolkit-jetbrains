...
abstract class DeleteResourceAction<in T : AwsExplorerResourceNode<*>> : SingleResourceNodeAction<T>, DumbAware {
    ...
    open val displayLongDelayWarning: Boolean = false
    ...
    final override fun actionPerformed(selected: T, e: AnActionEvent) {
        ...
        val response = DeleteResourceDialog(selected.nodeProject, resourceType, resourceName, this.displayLongDelayWarning).showAndGet()
        if (!response) {
