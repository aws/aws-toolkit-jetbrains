...
class DefaultExecutableManager : PersistentStateComponent<ExecutableStateList>, ExecutableManager {
    ...
    override fun getExecutableIfPresent(type: ExecutableType<*>): ExecutableInstance {
        ...
        if (instance == null) {
            ...
            return ExecutableInstance.UnresolvedExecutable(message("executableCommon.missing_executable", type.displayName, message("executableCommon.not_installed")))
        }
    ...
    private fun resolve(type: ExecutableType<*>): ExecutableInstance = try {
        (type as? AutoResolvable)?.resolve()?.let { validateAndSave(type, it, autoResolved = true) }
            ?: ExecutableInstance.UnresolvedExecutable(message("executableCommon.missing_executable", type.displayName, message("executableCommon.not_installed")))
    } catch (e: Exception) {
