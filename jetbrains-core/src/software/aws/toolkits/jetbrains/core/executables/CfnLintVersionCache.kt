...
object CfnLintVersionCache : FileInfoCache<SemVer>() {
    override fun getFileInfo(path: String): SemVer {
        val executableName = "cfn-lint"
        ...
        if (process.exitCode != 0) {
            ...
            var output: String = process.stdout.trimEnd()
            if (output.isEmpty()) {
                throw IllegalStateException(message("executableCommon.empty_info", executableName))
            }
            output = output.replace(executableName, "").trimStart()
            return SemVer.parseFromText(output)
                ?: throw IllegalStateException(
                    message("executableCommon.version_parse_error",
                    executableName,
                    output
                )
                )
