...
class OpenReadOnlyFileAction : SingleExplorerNodeAction<DynamicResourceNode>(message("dynamic_resources.openReadOnlyFile_text")), DumbAware {
    override fun actionPerformed(selected: DynamicResourceNode, e: AnActionEvent) {
        DynamicResourceFileManager.getInstance(selected.nodeProject).openEditor(selected.identifier(), OpenResourceMode.READ)
    }
}
