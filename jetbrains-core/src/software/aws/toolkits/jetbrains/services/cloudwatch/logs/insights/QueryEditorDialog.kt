...
class QueryEditorValidator{
    fun validateEditorEntries(view:QueryEditor):ValidationInfo?{
        ...
        if (view.absoluteTimeRadioButton.isSelected && view.qStartDate.date==null){return ValidationInfo(message("cloudwatch.logs.no.start_date"),view.qStartDate)}
        if (view.absoluteTimeRadioButton.isSelected && view.qEndDate.date==null){return ValidationInfo(message("cloudwatch.logs.no.end_date"),view.qEndDate)}
        if (view.relativeTimeRadioButton.isSelected && view.RelativeTimeNumber.text.isEmpty()){return ValidationInfo(message("cloudwatch.logs.no.relative_time_number"),view.RelativeTimeNumber)}
        if(view.absoluteTimeRadioButton.isSelected && view.qStartDate.date>view.qEndDate.date){return ValidationInfo(message("cloudwatch.logs.compare.start.end.date"),view.qStartDate)}
        if (!view.queryLogGroupsRadioButton.isSelected && !view.searchTerm.isSelected){
            return ValidationInfo(message("cloudwatch.logs.no.query.selected"),view.searchTerm)}
        if(view.queryLogGroupsRadioButton.isSelected && view.queryBox.text.isEmpty()){
            return ValidationInfo(message("cloudwatch.logs.no.query.entered"),view.queryBox)
        }
        if(view.searchTerm.isSelected && view.querySearchTerm.text.isEmpty()){
            return ValidationInfo(message("cloudwatch.logs.no.term.entered"),view.querySearchTerm)
        }
        return null
    }
}
