...
class CodeWhispererExplorerActionManager : PersistentStateComponent<CodeWhispererExploreActionState> {
    ...
    fun checkActiveCodeWhispererConnectionType(project: Project) =
        ToolkitConnectionManager.getInstance(project).activeConnectionForFeature(CodeWhispererConnection.getInstance())?.let { conn ->
            val connState = conn.state()
            when (connState) {
...
private fun ToolkitConnection.state(): BearerTokenAuthState =
    ((this as? AwsBearerTokenConnection)?.getConnectionSettings()?.tokenProvider?.delegate as? BearerTokenProvider)?.state()
        ?: BearerTokenAuthState.NOT_AUTHENTICATED
...
fun isCodeWhispererExpired(project: Project) = with(CodeWhispererExplorerActionManager.getInstance()) {
    if (checkActiveCodeWhispererConnectionType(project) == CodeWhispererLoginType.Logout) {
        return@with null
    }
    return@with checkActiveCodeWhispererConnectionType(project) == CodeWhispererLoginType.Expired
}
