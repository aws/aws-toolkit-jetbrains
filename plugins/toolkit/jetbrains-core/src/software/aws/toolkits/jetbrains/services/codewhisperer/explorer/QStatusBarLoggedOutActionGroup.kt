...
class QStatusBarLoggedOutActionGroup : DefaultActionGroup() {
    private val loggedInActionProvider = object : ActionProvider<AnAction> {
    ...
    override fun getChildren(e: AnActionEvent?) = e?.project?.let {
        buildList {
            ...
            addAll(buildActionListForInlineSuggestions(it, loggedInActionProvider))

            add(Separator.create())
            add(Separator.create(message("codewhisperer.statusbar.sub_menu.other_features.title")))
            addAll(buildActionListForOtherFeatures(it, loggedInActionProvider))

            add(Separator.create())
            add(Separator.create(message("codewhisperer.statusbar.sub_menu.connect_help.title")))
            addAll(buildActionListForConnectHelp(loggedInActionProvider))
