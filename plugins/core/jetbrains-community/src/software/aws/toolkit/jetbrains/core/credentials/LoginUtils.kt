...
sealed class Login<T> {
    ...
    fun login(project: Project): T {
        ...
        try {
            check(!ToolkitOAuthService.Companion.getInstance().hasPendingRequest()) {
                LOG.warn { "$this attempt initiated with pending request: ${ToolkitOAuthService.Companion.getInstance().pendingRequest()}" }
