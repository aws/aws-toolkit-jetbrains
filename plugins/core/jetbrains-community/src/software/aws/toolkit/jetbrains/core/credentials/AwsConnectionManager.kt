...
abstract class AwsConnectionManager(private val project: Project) : SimpleModificationTracker(), Disposable {
    ...
    private val regionProvider = AwsRegionProvider.Companion.getInstance()
        ...
        pluginAwareExecuteOnPooledThread {
            ...
            try {
                val credentialsProvider =
                    CredentialManager.getInstance().getAwsCredentialProvider(credentialsIdentifier, region)
                ...
                LOGGER.warn(e) {
                    AwsCoreBundle.message(
                        "credentials.profile.validation_error",
                        credentialsIdentifier.displayName
                    )
                }
    ...
    val activeRegion: AwsRegion
        get() = selectedRegion ?: AwsRegionProvider.Companion.getInstance().defaultRegion().also {
    ...
    companion object {
        ...
        val AwsConnectionManager.selectedPartition get() = selectedRegion?.let { AwsRegionProvider.Companion.getInstance().partitions()[it.partitionId] }
    }
}
...
sealed class ConnectionState(val displayMessage: String, val isTerminal: Boolean) {
    ...
    protected val editCredsAction by lazy { ActionManager.getInstance().getAction("aws.toolkit.settings.upsertCredentials") }
