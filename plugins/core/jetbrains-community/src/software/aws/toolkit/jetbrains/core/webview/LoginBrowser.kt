...
import software.aws.toolkit.jetbrains.core.credentials.Login
...
import software.aws.toolkit.jetbrains.core.credentials.sso.PendingAuthorization
import software.aws.toolkit.jetbrains.core.credentials.sso.bearer.InteractiveBearerTokenProvider
import software.aws.toolkit.jetbrains.core.credentials.ssoErrorMessageFromException
...
import software.aws.toolkit.jetbrains.utils.pollFor
...
abstract class LoginBrowser(
    ...
    private fun isReAuth(scopes: List<String>, requestedStartUrl: String): Boolean = ToolkitConnectionManager.Companion.getInstance(project)
        .let {
            val qConnection = it.activeConnectionForFeature(QConnection.Companion.getInstance()) as AwsBearerTokenConnection?
            val codeCatalystConnection = it.activeConnectionForFeature(CodeCatalystConnection.Companion.getInstance()) as AwsBearerTokenConnection?
    ...
    private suspend fun pollForAuthorization(provider: InteractiveBearerTokenProvider): PendingAuthorization? =
        pollFor {
        ...
        if (connection is AwsBearerTokenConnection) {
            loginWithBackgroundContext {
                reauthConnectionIfNeeded(
                    ...
                    onPendingToken,
                    isReAuth = true,
                    reauthSource = ReauthSource.LOGIN_BROWSER
                )
            }
            stopAndClearBrowserOpenTimer()
        }
