...
package software.aws.toolkit.jetbrains.core.credentials.sso.pkce
...
import software.aws.toolkit.jetbrains.core.credentials.sso.PKCEClientRegistration
import software.aws.toolkit.jetbrains.core.credentials.sso.bearer.buildUnmanagedSsoOidcClient
        ...
        val token = try {
            buildUnmanagedSsoOidcClient(registration.region).use { client ->
                client.createToken {
                    ...
                    val duration = Duration.between(startTime, Instant.now()).toMillis().toDouble()
