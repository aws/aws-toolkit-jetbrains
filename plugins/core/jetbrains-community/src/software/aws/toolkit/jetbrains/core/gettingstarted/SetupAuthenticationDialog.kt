...
import software.aws.toolkit.jetbrains.core.credentials.messageFromConfigFacadeError
...
data class SetupAuthenticationDialogState(
    ...
    data class IdentityCenterTabState(
        ...
        var startUrl: String = "",
        var region: AwsRegion = AwsRegionProvider.Companion.getInstance().defaultRegion(),
        ...
        when (selectedTab()) {
            SetupAuthenticationTabs.IDENTITY_CENTER -> {
                ...
                if (!promptForIdcPermissionSet) {
                    ToolkitConnectionManager.Companion.getInstance(project).switchConnection(connection)
                    close(OK_EXIT_CODE)
                    return
                }
            ...
            SetupAuthenticationTabs.IAM_LONG_LIVED -> {
                ...
                val callerIdentity = tryOrNull {
                    runUnderProgressIfNeeded(
                        project,
                        AwsCoreBundle.message("settings.states.validating.short"),
                        cancelable = true
                    ) {
                        AwsClientManager.Companion.getInstance().createUnmanagedClient<StsClient>(
                            StaticCredentialsProvider.create(
                                AwsBasicCredentials.create(
                                    state.iamTabState.accessKey,
                                    state.iamTabState.secretKey
                                )
                            ),
                            Region.AWS_GLOBAL
                        ).use { client ->
    ...
    private fun iamTab() = panel {
        row {
            text(AwsCoreBundle.message("gettingstarted.setup.iam.notice")) { hyperlinkEvent ->
                ...
                ActionManager.getInstance().getAction("aws.toolkit.settings.upsertCredentials").actionPerformed(actionEvent)
            }
        }
...
class IdcTabPanelBuilder(
    ...
    fun build() = panel {
        ...
        row(AwsCoreBundle.message("gettingstarted.setup.idc.region")) {
            comboBox(
                AwsRegionProvider.Companion.getInstance().allRegionsForService("sso").values,
