...
fun checkBearerConnectionValidity(project: Project, source: BearerTokenFeatureSet): ActiveConnection {
    ...
    val activeConnection = when (source) {
        BearerTokenFeatureSet.CODECATALYST -> ToolkitConnectionManager.Companion.getInstance(project).activeConnectionForFeature(
            CodeCatalystConnection.Companion.getInstance()
        )
        BearerTokenFeatureSet.Q -> ToolkitConnectionManager.Companion.getInstance(project).activeConnectionForFeature(
            QConnection.Companion.getInstance()
        )
...
fun checkIamConnectionValidity(project: Project): ActiveConnection {
    val currConn = AwsConnectionManager.Companion.getInstance(project).selectedCredentialIdentifier ?: return ActiveConnection.NotConnected
    val invalidConnection = AwsConnectionManager.Companion.getInstance(project).connectionState.let { it.isTerminal && it !is ConnectionState.ValidConnection }
