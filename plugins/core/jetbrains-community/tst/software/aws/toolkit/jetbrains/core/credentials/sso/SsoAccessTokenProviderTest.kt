...
import software.aws.toolkit.jetbrains.utils.rules.SsoLoginCallbackProviderRule
...
class SsoAccessTokenProviderTest {
    ...
    fun `initiates authorizatation_grant registration when requested in a commercial region`() {
        setPkceTrue()
        val sut = SsoAccessTokenProvider(
            ssoUrl,
            "us-east-1",
            ssoCache,
            ssoOidcClient,
    ...
    fun `refresh access token updates caches`() {
        ...
        val accessToken =
            DeviceAuthorizationGrantToken(ssoUrl, ssoRegion, "dummyToken", "refreshToken", clock.instant())
        ssoCache.stub {
            on(
                ssoCache.loadAccessToken(argThat<DeviceGrantAccessTokenCacheKey> { startUrl == ssoUrl })
            ).thenReturn(
