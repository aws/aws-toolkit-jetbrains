...
class WorkspaceTest : LightPlatformTestCase() {
    ...
    private fun createDir(relativePath: String): VirtualFile {
        ...
        return if (normalizedPath.isEmpty()) {
            tempDir.refreshAndFindVirtualDirectory()!!
        } else {
            tempDir.resolve(normalizedPath).createDirectories().refreshAndFindVirtualDirectory()!!
        }
    }
    ...
    private fun createFile(path: String): VirtualFile {
        ...
        return Files.createFile(filePath).refreshAndFindVirtualFile()!!
    }
