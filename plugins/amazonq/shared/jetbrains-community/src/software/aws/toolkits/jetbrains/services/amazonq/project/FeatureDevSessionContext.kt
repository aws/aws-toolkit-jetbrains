...
open class FeatureDevSessionContext(val project: Project, val maxProjectSizeBytes: Long? = null) {
    ...
    private val workspaceContentRoots = findWorkspaceContentRoots()
    ...
    private val changeListManagers = ProjectManager.getInstance().openProjects.map { ChangeListManager.getInstance(it) }
    ...
    private fun shouldIncludeInZipFile(file: VirtualFile, isAutoBuildFeatureEnabled: Boolean): Boolean {
        ...
        if (!isWorkspaceSourceContent(file, workspaceContentRoots, changeListManagers, additionalGlobalIgnoreRules)) {
            return false
        }
     ...
     * (i.e. We could change from workspaceRoot to selectionRoot here, and these use cases would change behavior in alignment with each other.)
     */
