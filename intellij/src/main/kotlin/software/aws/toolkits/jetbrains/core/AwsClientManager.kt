...
class AwsClientManager(private val project: Project) {
    ...
    companion object {
        private val serviceClientBuilder = mapOf<KClass<*>, SyncClientBuilder<*, *>>(
            S3Client::class to S3Client.builder(),
            LambdaClient::class to LambdaClient.builder()
        )
    ...
    fun <T : Any> getClient(clz: KClass<T>): T {
        val key = AwsClientKey(
            ...
            regionId = settings.currentRegion.name,
            serviceClass = clz
        )
    ...
    private fun <T : Any> createNewClient(key: AwsClientKey): T {
        @Suppress("UNCHECKED_CAST")
        return serviceClientBuilder.getValue(key.serviceClass)
            .credentialsProvider(getCredentialsProvider(settings.currentProfile!!.name).toV2())
            .region(Region.of(settings.currentRegion.id))
